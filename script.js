...
window.Heritage = (function(){
  ...
  function initCommon(){
    ...
    // Logo image fallback: if Assets/Heritage.png hasn't been uploaded yet (or fails to load),
    // hide the broken-image icon and let the text wordmark stand alone.
  ...
  function renderHomePage(content, root){
    ...
    if(collectionsGrid && content.collections){
      collectionsGrid.innerHTML = content.collections.map(c=>{
        ...
        return '<div class="collection-card"><div class="motif">'+motifSVG(c.motif)+'</div><div class="scrim"></div>'+
    ...
    if(projectsGrid && content.projects){
      projectsGrid.innerHTML = content.projects.map(p=>
        '<div class="project-item" data-cat="'+p.cat+'"><div class="motif" style="--ar:'+p.aspect+'">'+motifSVG(p.motif,'#D4B876')+'</div>'+
      ...
      if(grid){
        grid.innerHTML = filtered.map(p=>{
          ...
          return '<div class="product-card">'+
            ...
            '<div class="motif">'+motifSVG(p.motif, swatchColor)+'</div></div>'+
